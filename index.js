...
// array of vehicles to sotre vehicles information (for cimplicity to avoid the use of database)

const vehicles = [
    {id: 1, license:'12-AA-32'},
    {id: 2, license:'43-BB-23'},
    {id: 3, license:'45-CC-23'},
];

// get() as two input auguments, 
//      first, is the path or the url e.g, '/' represents the 
//      second, a callbabk function that must have two arguments (req, res) request and response
// look on req, res properties in node express documentation: e.g., http://expressjs.com/en/5x/api.html#req
app.get('/',  (req, res) => {
...
// app.put method allow to update a new vehicle by receiving a new license place.
app.put('/api/vehicles/:id', (req, res) => {
    ...
    const {error} = validadeVehicle(req.body); // result.error object destructuring
...
function validadeVehicle(vehicle){
    const schema = {
        license: Joi.string().min(8).max(8).required()
    };
